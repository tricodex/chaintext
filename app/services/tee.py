...
class OnChainVerifier:
    ...
    def __init__(self, web3_provider: Optional[str] = None):
        ...
        logger.info(f"Using Flare vTPM contract address: {self.flare_vtpm_address}")
        ...
        try:
            ...
            if self.tee_verifier_address and self.tee_verifier_address != "0x0000000000000000000000000000000000000000":
                try:
                    ...
                    checksummed_address = self.web3.to_checksum_address(self.tee_verifier_address)
                    self.tee_verifier = self.web3.eth.contract(
                        address=checksummed_address,
                        abi=self.tee_verifier_abi
                    )
                    logger.info(f"Initialized TEE verifier contract at {checksummed_address}")
                except Exception as e:
                    logger.error(f"Failed to initialize TEE verifier contract: {e}")
        ...
        try:
            ...
            if self.flare_vtpm_address and self.flare_vtpm_address != "0x0000000000000000000000000000000000000000":
                try:
                    # Use checksummed address
                    checksummed_address = self.web3.to_checksum_address(self.flare_vtpm_address)
                    self.flare_vtpm_contract = self.web3.eth.contract(
                        address=checksummed_address,
                        abi=self.flare_vtpm_abi
                    )
                    logger.info(f"Initialized Flare vTPM Attestation contract at {checksummed_address}")
                except Exception as e:
                    logger.error(f"Failed to initialize Flare vTPM contract: {e}")
        ...
        try:
            ...
            if attestation_type == "gcp_vtpm" and self.flare_vtpm_contract:
                try:
                    ...
                    try:
                        # This will succeed even if no quotes are registered
                        self.flare_vtpm_contract.functions.getRegisteredQuote(self.web3.to_checksum_address(self.flare_vtpm_address)).call()
                        logger.debug("Successfully accessed the FlareVtpmAttestation contract")
                    except Exception as access_error:
                        logger.error(f"Error accessing contract: {access_error}")
                        raise RuntimeError(f"Contract access failed: {access_error}")
                    
                    # Parse header to get token type
                    try:
                        # Check if token verifier is set - this is a common initialization mistake
                        # We create our own simple header parsing implementation
                        header_bytes = bytes.fromhex(header[2:]) if header.startswith("0x") else bytes.fromhex(header)
                        header_json = json.loads(header_bytes.decode('utf-8'))
                        
                        # Determine token type based on header
                        token_type = "PKI" if "x5c" in header_json else "OIDC"
                        logger.debug(f"Token type determined from header: {token_type}")
                        
                        # Try to check if a verifier is set for this token type
                        try:
                            # This call will tell us if the token verifier is configured
                            token_type_bytes = token_type.encode('utf-8')
                            verifier_addr = self.flare_vtpm_contract.functions.tokenTypeVerifiers(token_type_bytes).call()
                            if verifier_addr == "0x0000000000000000000000000000000000000000":
                                logger.error(f"No token verifier set for type: {token_type}")
                                raise RuntimeError(f"Contract not initialized: No verifier for {token_type}")
                            logger.debug(f"Found token verifier at {verifier_addr}")
                        except Exception as verifier_error:
                            logger.error(f"Error checking token verifier: {verifier_error}")
                            raise RuntimeError(f"Contract initialization check failed: {verifier_error}")
                    except Exception as parse_error:
                        logger.error(f"Error parsing header: {parse_error}")
                        # Continue anyway, as the contract call will reveal the real issue

                    # Call the verifyAndAttest function (as read-only call)
                    logger.debug(f"Calling contract with header length: {len(header)}, payload length: {len(payload)}, signature length: {len(signature)}")
                    ...
                    try:
                        ...
                        if "InvalidVerifier" in str(call_error):
                            logger.error("Contract error: Token verifier not configured. The contract owner must call setTokenTypeVerifier().")
                            raise RuntimeError("Contract not fully initialized: Missing token verifier configuration")
                        elif "PayloadValidationFailed" in str(call_error):
                            logger.error("Contract error: Payload validation failed. The baseQuoteConfig may not match the token.")
                            raise RuntimeError("Attestation validation failed: Token doesn't match required configuration")
                        else:
                            logger.error(f"Contract call error: {call_error}")
                            raise RuntimeError(f"Contract call failed: {call_error}")
                    
                except Exception as vTPM_err:
                    logger.error(f"Error calling FlareVtpmAttestation contract: {vTPM_err}")
                    # Use more detailed fallback message
                    logger.info("Falling back to simulated verification due to contract error")
            ...
            # Handle regular TPM attestation
                ...
                try:
                    ...
                    logger.info("Falling back to simulated verification due to contract call error")
